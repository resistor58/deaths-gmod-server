...
function ENT:Initialize()
	...
	self.Entity:SetModel("models/weapons/p_garand_rg_grenade.mdl")
	...
	if not self.Entity:GetNWBool("Live") then
		self.Timer = CurTime() + 0.075
	else
		self.Timer = CurTime() + 0.25
	end
...
/*
	if activator:GetActiveWeapon():GetClass() == "weapon_mad_dod_garand_rg_live" then return end

	self.Entity:Remove()

	if (activator:IsPlayer()) then
		activator:Give("weapon_mad_dod_garand_rg_live")
		activator:SelectWeapon("weapon_mad_dod_garand_rg_live")
	end
*/
end
