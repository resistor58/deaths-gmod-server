...
SWEP.IronSightsPos 		= Vector (-3.6183, -3.6069, 0.5514)
SWEP.IronSightsAng 		= Vector (0.0225, 0.0434, 0)
SWEP.RunArmOffset 		= Vector (3.5308, 0, 1.2151)
SWEP.RunArmAngle 			= Vector (-14.2869, 20.5797, 0)
...
function SWEP:Precache()
    	...
    	util.PrecacheSound("weapons/mg42_shoot.wav")
end
...
function SWEP:Reload()
	...
	self.Weapon:SetNWInt("NextChangeMode", CurTime() + 5.25)
...
function SWEP:ShootAnimation()
	...
	if (self.Weapon:GetDTBool(3)) then
		...
		if self.Weapon:Clip1() < 9 then
			...
			Animation:SetSequence(Animation:LookupSequence("downshoot_" .. math.random(1, 3)))
		end
	else
		self.Weapon:SendWeaponAnim(ACT_VM_IDLE)
		local Animation = self.Owner:GetViewModel()
