...
XQMHydraulicModel = "models/jaanus/wiretool/wiretool_siren.mdl"


if CLIENT then
    language.Add( "Tool_xqm_wire_hydraulic_name", "Hydraulic Tool (Wire)" )
    language.Add( "Tool_xqm_wire_hydraulic_desc", "Makes a controllable hydraulic" )
    language.Add( "Tool_xqm_wire_hydraulic_0", "Primary: Place hydraulic\nSecondary: Place hydraulic along the hit normal" )
    language.Add( "Tool_xqm_wire_hydraulic_1", "Left click on the second point" )
    language.Add( "Tool_xqm_wire_hydraulic_2", "Left click to place the controller" )
    language.Add( "XQMWireHydraulicTool_width", "Width:" )
    language.Add( "XQMWireHydraulicTool_material", "Material:" )
	language.Add( "XQMWireHydraulicTool_fixed", "Fixed:" )
	language.Add( "undone_xqmwirehydraulic", "Undone Wire Hydraulic" )
end
...
function TOOL:LeftClick( trace )
	...
	if ( iNum > 1 ) then
		...
		Msg("Loaded model is: " ..self:GetClientInfo( "model" ).." \n")
		XQMHydraulicModel = model
		...
		local controller = MakeXQMWireHydraulicController(ply, trace.HitPos, Ang, nil, const, rope)
...
if SERVER then
	...
	function MakeXQMWireHydraulicController( pl, Pos, Ang, MyEntId, const, rope )
		local controller = ents.Create("gmod_xqm_wire_hydraulic")
		
		controller:SetPos( Pos )
		controller:SetModel( XQMHydraulicModel )
		//Msg( ..controller:SetModel( model ).. "\n")
	...
	duplicator.RegisterEntityClass("gmod_xqm_wire_hydraulic", MakeXQMWireHydraulicController, "Pos", "Ang", "MyId")
	...
	duplicator.RegisterConstraint( "XQMWireHydraulic", MakeWireHydraulic, "pl", "Ent1", "Ent2", "Bone1", "Bone2", "LPos1", "LPos2", "width", "material", "fixed", "MyCrtl" )
...
function TOOL:Reload( trace )
	...
	local  bool = constraint.RemoveConstraints( trace.Entity, "XQMWireHydraulic" )
	return bool
...
function TOOL.BuildCPanel(panel)
	...
	panel:AddControl( "PropSelect", {
		Label = "Model",
		ConVar = "xqm_wire_hydraulic_model",
		Category = "HydControllers",
		Models = list.Get( "HydraulicControllers" ) } )
...
list.Set( "HydraulicControllers", "models/XQM/HydControlBox.mdl", {} )
list.Set( "HydraulicControllers", "models/jaanus/wiretool/wiretool_siren.mdl", {} )
