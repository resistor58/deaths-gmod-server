...
function()
	...
	for i=1, 10 do
		...
		if KeyEvents[i] == 1 then
			RunConsoleCommand( "SetSCarPlayerWepKey", (i-1) )	
		end			
	end
	...
	if KeyEvents[11] == 1 then
		RunConsoleCommand( "SetSCarPlayerWepKey", 10 )	
	end	
	
	--V Key
	if( input.IsKeyDown(32)) then
		if(KeyEvents[12]==0) then KeyEvents[12] = 1
		elseif(KeyEvents[12]==1) then KeyEvents[12] = 2
		elseif(KeyEvents[12]==2) then KeyEvents[12] = 2
		elseif(KeyEvents[12]==3) then KeyEvents[12] = 1 end
	else
		if(KeyEvents[12]==0) then KeyEvents[12] = 0
		elseif(KeyEvents[12]==1) then KeyEvents[12] = 3
		elseif(KeyEvents[12]==2) then KeyEvents[12] = 3
		elseif(KeyEvents[12]==3) then KeyEvents[12] = 0 end
	end	

	if KeyEvents[12] == 1 then
		RunConsoleCommand( "SetSCarPlayerWepKey", 11 )	
	end		
